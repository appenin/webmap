...
import { StrictValue, StyleConfig, StyledComponent, StyledComponentProps } from './types';

export function styled<
  TComponent extends keyof JSX.IntrinsicElements | React.ComponentType<any>,
  TConfigs extends Array<string | { [key: string]: unknown }>,
>(
      ...
      :
          | {
              ...
              forwardProps?: 'variants' extends keyof TConfigs[K]
                ? Array<keyof TConfigs[K]['variants']>
                : [];
            }
          | string;
  }
): StyledComponent<TComponent, StyledComponentProps<TConfigs>> & React.RefAttributes<TComponent> {
...
function getDisplayName(component: keyof JSX.IntrinsicElements | React.ComponentType<any>) {
  const innerName =
...
type PreparedConfig = {
  ...
  compoundVariants: Array<{ [key: string]: unknown } & { className: string[] }>;
};
