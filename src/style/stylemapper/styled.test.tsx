...
describe('styled', () => {
  const TestComponent = (props: { open?: boolean; className?: string }) => {
    return <div className={props.className}>{props.open ? 'open' : 'closed'}</div>;
  };
  ...
  it('forwards props that have no variant classes', () => {
    ...
    const StyledComponent = styled(TestComponent, {});
    const { getByText } = render(<StyledComponent open={true} className={className} />);
  ...
  it('adds className via configuration + prop', () => {
    ...
    const StyledComponent = styled(TestComponent, className);
    const { container } = render(<StyledComponent className={otherClassName} />);
    expect(container.firstChild).toHaveClass(className);
  ...
  it('merges multiple variant configs', () => {
    ...
    const { container } = render(<StyledComponent prop1={'value1'} prop2={'value2'} />);
    expect(container.firstChild).toHaveClass(class1);
  ...
  it('merges variant config + class config + prop', () => {
    ...
    const { container } = render(<StyledComponent className={class3} prop1={'value1'} />);
    expect(container.firstChild).toHaveClass(class1);
    expect(container.firstChild).toHaveClass(class2);
  ...
  it('creates component from itrinsic HTML element key', () => {
    const elementNames: Array<keyof JSX.IntrinsicElements> = [
      'div',
      'span',
      'h1',
      'h2',
  ...
  it('does not add classes from unmatched compound variants', () => {
    ...
    const { container } = render(<StyledComponent prop1={'value2'} prop2={'value2'} />);
    expect(container.firstChild).toHaveClass(class1);
    expect(container.firstChild).toHaveClass(class2);
